function feature_output = voxel_surface_SUV_product_1(varargin)
global image_global;
...
if exist('mask_for_TA')==1
    ...
    img_in2 = zeros(size(img_in)+2); img_in2(2:end-1, 2:end-1, 2:end-1) = img_in; img_in = img_in2; 
    area_sum = 0;
    counter = 0;
    surf_act = [];
            ...
            for idx3 = 2:size(mask, 3)-1
                if mask(idx1, idx2, idx3)==1
                    if prod([mask(idx1-1, idx2, idx3) mask(idx1+1, idx2, idx3) mask(idx1, idx2-1, idx3) mask(idx1, idx2+1, idx3) mask(idx1, idx2, idx3-1) mask(idx1, idx2, idx3+1)])==1
                        ...
                        surf_act(end+1) = img_in2(idx1, idx2, idx3);
                        counter = counter+1;
                    end
                end
            end
    ...
    feature_out = feature_output* tumor_surface_area_CGITA;
else
