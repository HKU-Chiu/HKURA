...
for idx = 1:ROI_n 
    ...
    name_list{idx} = current_ROI_obj.ROIName;    
end
...
for idx = 1:ROI_n 
    ...
    ROI_struct(counter).roiNumber        = current_ROI_obj.ReferencedROINumber;
    ROI_struct(counter).structureName = name_list{idx};
    ...
    ROI_struct(counter).contour = temp1.contour;
end

ROI_struct(end+1).roiNumber = ROI_n+1;
ROI_struct(end).structureName = name_list{end};

for idx1 = 1:ROI_n 
    current_ROI_obj                        = getfield(dicom_hdr.ROIContourSequence, names_field1{idx1});
    if isfield(current_ROI_obj, 'ContourSequence')
        temp1= align_contour_on_slices(current_ROI_obj.ContourSequence, fliplr(-handles.Primary_image_obj.zV), ...
            -fliplr(handles.Primary_image_obj.yV), handles.Primary_image_obj.xV, fliplr(-handles.Primary_image_obj.zV), ...
            fliplr(handles.Primary_image_obj.xV), fliplr(handles.Primary_image_obj.yV), fliplr(-handles.Primary_image_obj.zV));
...
%% code backup
%          temp1= align_contour_on_slices(current_ROI_obj.ContourSequence, fliplr(-handles.Primary_image_obj.zV), ...
%              (handles.Primary_image_obj.xV),       (handles.Primary_image_obj.yV),       fliplr(handles.Primary_image_obj.zV), ...
%              -fliplr(handles.Primary_image_obj.xV), fliplr(handles.Primary_image_obj.yV), fliplr(-handles.Primary_image_obj.zV));
%         temp1= align_contour_on_slices(current_ROI_obj.ContourSequence, fliplr(-handles.Primary_image_obj.zV), ...
%             fliplr(handles.Primary_image_obj.yV), handles.Primary_image_obj.xV, fliplr(-handles.Primary_image_obj.zV), ...
%             -fliplr(handles.Primary_image_obj.xV), fliplr(handles.Primary_image_obj.yV), fliplr(-handles.Primary_image_obj.zV));
