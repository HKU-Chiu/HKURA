function ROI_struct = XML_VOI_reader_match_image(filename, handles);%, zV, x1, y1, z1, x2, y2, z2)
...
h = msgbox('Reading the XML VOI file. Please wait...');
...
slice_vec = length(handles.zV_original):-1:1;
fid = fopen(filename);
tol = 1e-4;
...
if ~isempty(strfind(text1, '<key>ROIPoints</key>'))
    text1 = text1(strfind(text1, '<key>ROIPoints</key>'):end);
end
    
for idx_list = 1%:n_list ==> assuming just one VOI per file now
    ...
    %end    
    n_roi = length(strfind(text1,'<key>Slice</key>'));
    if n_roi == 0
        n_roi = 1;
    end
    vertices_counter = n_roi;
    text2 = text1;
    for idx_roi = 1:n_roi
        pos1 = strfind(text2, '<array>');
        pos2 = strfind(text2, '</array>');        
        text3 = text2(pos1(1)+7:pos2(1)-1);
        pos3 = strfind(text3, '<string>');
        pos4 = strfind(text3, '</string>');
        clear temp_roi_mat temp_roi_mat2;
        for idx_string = 1:length(pos3)
        	temp_roi_mat(:,idx_string)=sscanf(text3(pos3(idx_string)+9:pos4(idx_string)-2),'%f,%f');
        end
        mask = roipoly_modified(zeros(length(xV), length(yV)), temp_roi_mat(1,:), temp_roi_mat(2,:));
        %mask = roipoly_modified(zeros(length(yV), length(xV)), point_mat2(:,1)-0.5, point_mat2(:,2)-0.5);
        B=bwboundaries(mask, 4, 'noholes');

        
         text3 = text2(pos1+7:end);
         pos1 = strfind(text3, '<integer>');
         slice_now = sscanf(text3(pos1+9:end), '%f');
        %temp_roi_mat2(1,:) = yV(round(temp_roi_mat(2,:)));
        %temp_roi_mat2(2,:) = xV(round(temp_roi_mat(1,:)));
        temp_roi_mat = B{1}';
        temp_roi_mat2(1,:) = xV(round(temp_roi_mat(1,:)));
        temp_roi_mat2(2,:) = yV(round(temp_roi_mat(2,:)));
        temp_roi_mat2(3, :) = zV(slice_now);
        temp_roi_mat = temp_roi_mat2;
        %text2 = text2(pos2(1)+8:end);

        all_pos_vertices{idx_roi}.pos_vertices = temp_roi_mat';
        all_pos_vertices{idx_roi}.idx_list = 1;
        all_pos_vertices{idx_roi}.slice_now = slice_vec(slice_now);
        all_pos_vertices{idx_roi}.contour_index = idx_roi;
        
        pos1 = strfind(text2, '<key>ROIPoints</key>');
        if length(pos1)>=2
            text2 = text2(pos1(2):end);
        end
    end
    %temp_str = get_value_from_text(text_list, '# voi_name', '%c');
    %temp_pos_str = strfind(temp_str, '# voi_name');
    
    ROI_struct(idx_list).structureName = 'XML';
...
parfor vertices_idx = 1:vertices_counter
    ...
    temp_parfor{vertices_idx} = all_pos_vertices{vertices_idx}.pos_vertices;
end
...
for vertices_idx = 1:vertices_counter
    ...
    %[vox_ind contour_img mask mask_ni] = convert_location_to_index(temp_parfor{vertices_idx}/10, handles.xV, handles.yV, handles.zV, handles);
    %subplot(1,2,1)
    %imagesc(handles.Primary_image_obj.image_volume_data(:,:,all_pos_vertices{vertices_idx}.slice_now)); axis off;
    %subplot(1,2,2)
    %imagesc(contour_img);
    %pause
end
