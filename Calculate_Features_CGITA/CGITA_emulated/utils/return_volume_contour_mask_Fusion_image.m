...
contour_volume = zeros(length(handles.Fusion_image_obj.xV), length(handles.Fusion_image_obj.yV), length(handles.Fusion_image_obj.zV));
    ...
    if ~isempty(contour(idx).segments)
        ...
        for idx2 = 1:n_segments
            ...
            if size(point_mat,1)>0
                [vox_ind contour_img mask] = convert_location_to_index(point_mat, handles.Fusion_image_obj.xV, handles.Fusion_image_obj.yV, handles.Fusion_image_obj.zV, handles);
