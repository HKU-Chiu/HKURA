...
%     end
    ...
    tempmat = reshape(current_item.ContourData, [3 length(current_item.ContourData)/3])';

    tempmat = tempmat(:, [2 1 3]);
    tempmat(:,[2 3]) = -tempmat(:, [2 3]);    
    ...
    %tempmat(:,[1 2]) = -tempmat(:, [1 2]);
    %tempmat(:,3) = fliplr(tempmat(:,3));
