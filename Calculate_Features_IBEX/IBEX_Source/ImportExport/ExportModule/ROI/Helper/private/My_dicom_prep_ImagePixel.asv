...
%Joy: replace size(X, 3) with SizeX3
SizeX3=1;

metadata(1).(dicom_name_lookup('0028', '0002')) = SizeX3;
metadata.(dicom_name_lookup('0028', '0004')) = getPhotometricInterp(X, map, txfr);

%Joy
if ~isfield(metadata, dicom_name_lookup('0028', '0010'))
    metadata.(dicom_name_lookup('0028', '0010')) = size(X, 1);     %Rows
    metadata.(dicom_name_lookup('0028', '0011')) = size(X, 2);     %Columns
end
...
if (~isequal(uid_details.Endian, uid_details.PixelEndian))
    ...
    out = DcmCast(in(:), 'uint8', true);
    out = DcmCast(out, in_class);
