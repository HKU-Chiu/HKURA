...
%   $Revision: 1.5.4.6 $  $Date: 2004/10/20 17:54:38 $
...
if (isempty(ol_bits))
    ...
    overlays = repmat(false, [info.Rows info.Columns info.NumberOfFrames ...
                    length(ol_bits)]);
...
if (info.PixelRepresentation == 1)
    ...
    switch (class(data))
    case 'int8'
        data = DcmCast(data, 'uint8');
    case 'int16'
        data = DcmCast(data, 'uint16');
    case 'int32'
        data = DcmCast(data, 'uint32');
    end
...
if (info.PixelRepresentation == 1)
    ...
    switch (class(data))
    case 'uint8'
        data = DcmCast(data, 'int8');
    case 'uint16'
        data = DcmCast(data, 'int16');
    case 'uint32'
        data = DcmCast(data, 'int32');
    end
