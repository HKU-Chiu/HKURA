...
%   $Revision: 1.4.4.9 $  $Date: 2004/08/10 01:44:44 $
...
% Group and Element
segment = DcmCast(uint16(attr.Group), 'uint8', swap);
segment = [segment DcmCast(uint16(attr.Element), 'uint8', swap)];
...
segment = [segment DcmCast(len, 'uint8', swap)];

if (ischar(attr.Data))
    segment = [segment uint8(attr.Data(:)')];
else
    tmp = DcmCast(attr.Data, 'uint8', swap);
    segment = [segment tmp(:)'];
end
...
tag = data(1:4);
tag = DcmCast(DcmCast(tag, 'uint16'), 'uint8', 1);

len = data(5:8);
len = DcmCast(DcmCast(len, 'uint32'), 'uint8', 1);
