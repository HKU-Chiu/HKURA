...
name = dicom_name_lookup('0008', '0064');        %Conversion Type
if (~isfield(metadata, name))
    metadata(1).(name) = 'WSD';
end

name = dicom_name_lookup('0018', '1016');     %SecondaryCaptureDeviceManufacturer
if (~isfield(metadata, name))
    metadata.(name) = 'The MathWorks';
end

name = dicom_name_lookup('0018', '1018');     %SecondaryCaptureDeviceManufacturerModelName
if (~isfield(metadata, name))
