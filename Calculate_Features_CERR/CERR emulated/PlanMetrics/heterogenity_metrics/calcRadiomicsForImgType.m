...
imageTypeC = {};
filtParamS = struct();
filtParamS.imageType = 'Original';
filtParamS.paramS = struct();
imageTypeC{1} = filtParamS;
k =1;


%--- Pre-process: 
%Generate volume based on original imageType
minIntensityCutoff = [];
maxIntensityCutoff = [];
if isfield(paramS.textureParamS,'minIntensityCutoff')
    minIntensityCutoff = paramS.textureParamS.minIntensityCutoff;
...
%features.Original contains struct with fields:
% - firstOrderS
% - shapeS
% - glcmFeatS
% - rlmFeatS
...
if 1%any([whichFeatS.glcm.flag,whichFeatS.glrlm.flag,whichFeatS.gtdm.flag, whichFeatS.gldm.flag,whichFeatS.glszm.flag])
