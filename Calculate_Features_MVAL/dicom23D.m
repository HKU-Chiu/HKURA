...
% 
% Modified version by: Jurgen van Lunenburg, The University of Hong Kong
...
function [volume_image, slice_data, image_meta_data] = ...
    ...
    %                 }
    try
        load(fullfile(dicom_directory, 'VOLUME_IMAGE.mat'));
        return;
    catch
    end
    ...
    listing = natsort({listing.name});
    N = numel(listing);
    assert(N>2,'Empty folder');
    ...
    if isfield(slice_data(1), 'SliceLocation')
        if ~any(cellfun(@isempty,{slice_data.SliceLocation}))
